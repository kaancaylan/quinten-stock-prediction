...
def modeling_prep(X_train, test_per_rets):
    ...
    to_drop = (
        test_per_rets[test_per_rets.isna()].index.get_level_values("symbol").unique()
    )
    ...
    y_train = y_train.groupby(level=1, group_keys=False).apply(
        lambda x: (1 + x).cumprod()[-1] - 1
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--validation_start", help="Date for the start of validation period", type=str
    )
    parser.add_argument(
        "--validation_end", help="Date for the start of validation period", type=str
    )
