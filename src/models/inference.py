...
def inference(
    inference_start: dt.datetime,
    inference_end: dt.datetime,
    prediction_start: dt.datetime,
    prediction_end: dt.datetime,
    model_path: str = None,
    df=None,
    model=None,
):
    ...
    try:
        ...
        y_test = y_test.groupby(level=1, group_keys=False).apply(
        ...
        return pd.DataFrame(
            {
                "y_test": y_test.loc[top20_stocks.index],
                "y_pred": y_pred.loc[top20_stocks.index],
            }
        )
    except KeyError:
        pass
